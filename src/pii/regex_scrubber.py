...
class RegexPIIScrubber:
    ...
    def __init__(self):
        ...
        self.phone_pattern = re.compile(
            r'(?:\+?[78]|\+?380|\+?995|\+?374|\+?998|\+?375|\+?1)'
            r'[\s\-\(]*(\d[\s]*\d[\s]*\d)[\s\-\)]*(\d[\s]*\d[\s]*\d)[\s\-]*(\d[\s]*\d)[\s\-]*(\d[\s]*\d)\b'
            r'|(?:\+\d{1,3}[\s\-]?)?\(?\d{3,4}\)?[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}\b',
            re.IGNORECASE
        )
        ...
        # TRON / TRC20 (T...)
        self.tron_pattern = re.compile(
            r'\bT[A-Za-z1-9]{33}\b'
        )
        # TON (EQ..., UQ...)
        self.ton_pattern = re.compile(
            r'\b(?:EQ|UQ)[a-zA-Z0-9_-]{46}\b'
        )

        # 4. API Keys and Tokens
        # OpenAI keys: sk-..., sk-proj-...
        self.openai_key_pattern = re.compile(
        ...
        self.tg_bot_token_pattern = re.compile(
        ...
        self.tg_invite_pattern = re.compile(
