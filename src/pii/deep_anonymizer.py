...
from src.pii.ner_scrubber import NERPIIScrubber, TECH_WHITELIST
...
COMMON_RU_FIRST_NAMES: Set[str] = {
    "александр", "алексей", "анатолий", "андрей", "антон", "артем", "артём", "артур",
    "богдан", "борис", "вадим", "валентин", "валерий", "василий", "виктор", "виталий",
    "владимир", "владислав", "вячеслав", "георгий", "глеб", "григорий", "даниил", "данил",
    "данила", "денис", "дмитрий", "евгений", "егор", "иван", "игорь", "илья", "кирилл",
    "константин", "лев", "леонид", "максим", "марк", "матвей", "михаил", "никита", "николай",
    "олег", "павел", "петр", "пётр", "роман", "ростислав", "руслан", "сергей", "станислав",
    "степан", "тимофей", "тимур", "федор", "фёдор", "филипп", "эдуард", "юрий", "ярослав",
    "алена", "алёна", "алина", "алиса", "алла", "анастасия", "анна", "антонина", "валентина",
    "валерия", "василиса", "вера", "вероника", "виктория", "галина", "дарья", "диана",
    "евгения", "екатерина", "елена", "елизавета", "жана", "жанна", "зинаида", "зоя", "инна",
    "ирина", "карина", "кира", "клавдия", "кристина", "ксения", "лариса", "лидия", "лилия",
    "любовь", "людмила", "маргарита", "марина", "мария", "марья", "надежда", "наталья",
    "наталия", "нина", "оксана", "олеся", "ольга", "полина", "райса", "раиса", "светлана",
    "снежана", "софия", "софья", "таисия", "тамара", "татьяна", "ульяна", "юлия", "яна"
}
...
class DeepPIIAnonymizer:
    ...
    def __init__(self, enable_ner: bool = True):
        ...
        self.db_url_pattern = re.compile(
            r'\b(?:postgres|postgresql|mysql|mongodb|redis|amqp)://[^\s]+\b',
            re.IGNORECASE
        )

        # Telegram forward / quote pattern: > [Name]: or Forwarded from Name
        self.tg_forward_pattern = re.compile(
            r'(?i)(?:переслано от|forwarded from)\s+([A-Za-zА-Яа-яЁё0-9_\-\s]{2,30})'
        )
    ...
    def _get_or_create_pseudonym(self, raw_id: str, raw_name: str) -> Tuple[str, str]:
        ...
        return self.user_id_to_anon[key], self.raw_name_to_anon.get(str(raw_name).strip(), f"Developer_{len(self.user_id_to_anon):05d}")
    ...
    def _harvest_name_inflections(self, raw_name: str):
        ...
        for w in words:
            ...
            if w_lower in COMMON_RU_FIRST_NAMES or (len(w) >= 3 and w[0].isupper() and any('\u0400' <= c <= '\u04FF' for c in w)):
