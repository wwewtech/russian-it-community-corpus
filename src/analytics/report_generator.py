...
class ReportGenerator:
    ...
    def export_markdown(self, output_path: Union[str, Path]) -> Path:
        ...
        md_lines.append(f"> **Дата генерации:** `{self.data.get('report_metadata', {}).get('generated_at', '')}` | **Версия движка:** `4.0.0-Enterprise`\n")

        # 1. Executive Summary & Valuation
        md_lines.append("## 🏆 1. ИТОГОВАЯ ОЦЕНКА КАЧЕСТВА И ГОТОВНОСТИ К ОБУЧЕНИЮ LLM")
        md_lines.append(f"**Общий балл:** `{val.get('total_score', 0)} / {val.get('max_score', 100)}` | **Категория:** **{val.get('quality_tier', '')}**")
        md_lines.append(f"\n*{val.get('tier_description', '')}*\n")
        md_lines.append(f"💰 **Рекомендуемая коммерческая оценка / эквивалент разработки:** `{val.get('recommended_valuation_range', '')}`\n")
        
        md_lines.append("| Критерий оценки | Балл | Макс | Статус |")
        md_lines.append("| :--- | :--- | :--- | :--- |")
        breakdown = val.get("score_breakdown", {})
        md_lines.append(f"| **Объём данных (Volume)** | {breakdown.get('volume_score', 0)} | 25 | {'🟢 Отлично' if breakdown.get('volume_score', 0) >= 20 else '🟡 Хорошо'} |")
        md_lines.append(f"| **Разнообразие авторов (Diversity)** | {breakdown.get('author_diversity_score', 0)} | 20 | {'🟢 Отлично' if breakdown.get('author_diversity_score', 0) >= 15 else '🟡 Хорошо'} |")
        md_lines.append(f"| **Техническая плотность (Domain Density)** | {breakdown.get('technical_density_score', 0)} | 20 | {'🟢 Отлично' if breakdown.get('technical_density_score', 0) >= 15 else '🟡 Хорошо'} |")
        md_lines.append(f"| **Диалоговая связность (Q&A Ratio)** | {breakdown.get('dialogue_continuity_score', 0)} | 15 | 🟢 Отлично |")
        md_lines.append(f"| **Лексическое разнообразие (Shannon Diversity)** | {breakdown.get('lexical_diversity_score', 0)} | 10 | 🟢 Отлично |")
        md_lines.append(f"| **Очистка PII и комплаенс** | {breakdown.get('pii_compliance_score', 0)} | 10 | 🟢 100% Cleared |")
        md_lines.append("\n---\n")
        ...
        md_lines.append(f"- **Временной диапазон:** `{v.get('date_start', '')}` — `{v.get('date_end', '')}` (`{v.get('total_days_active', 0)}` дней)")
        md_lines.append(f"- **Средняя интенсивность:** `{v.get('messages_per_day', 0)}` сообщений / день (`{v.get('tokens_per_day', 0):,}` токенов / день)")
        md_lines.append(f"- **Общее число слов:** `{v.get('total_words', 0):,}`")
        md_lines.append(f"- **Оценка объёма токенов (BPE):** `{v.get('total_tokens_estimated', 0):,}` токенов (~`{v.get('total_tokens_estimated', 0) / 1_000_000:.2f}M`)")
        md_lines.append(f"- **Уникальный словарь (Леммы/Слова):** `{v.get('vocabulary_unique_words', 0):,}`")
        md_lines.append(f"- **Индекс лексического разнообразия (Shannon):** `{l.get('shannon_entropy', 0):.2f}`\n")
        ...
        md_lines.append("| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |")
        md_lines.append(f"| **Символов** | {char_dist.get('mean', 0)} | {char_dist.get('median', 0)} | {char_dist.get('min', 0)} | {char_dist.get('max', 0)} | {char_dist.get('p25', 0)} | {char_dist.get('p75', 0)} | {char_dist.get('p90', 0)} | {char_dist.get('p99', 0)} |")
        md_lines.append(f"| **Слов** | {word_dist.get('mean', 0)} | {word_dist.get('median', 0)} | {word_dist.get('min', 0)} | {word_dist.get('max', 0)} | {word_dist.get('p25', 0)} | {word_dist.get('p75', 0)} | {word_dist.get('p90', 0)} | {word_dist.get('p99', 0)} |")
        md_lines.append(f"| **Токенов** | {tok_dist.get('mean', 0)} | {tok_dist.get('median', 0)} | {tok_dist.get('min', 0)} | {tok_dist.get('max', 0)} | {tok_dist.get('p25', 0)} | {tok_dist.get('p75', 0)} | {tok_dist.get('p90', 0)} | {tok_dist.get('p99', 0)} |")
        md_lines.append("\n---\n")
        ...
        md_lines.append(f"- **Положительных сообщений:** `{sent_stat.get('positive', 0):,}` (`{sent_stat.get('pos_ratio', 0):.1f}%`)")
        md_lines.append(f"- **Отрицательных сообщений:** `{sent_stat.get('negative', 0):,}` (`{sent_stat.get('neg_ratio', 0):.1f}%`)")
        md_lines.append(f"- **Нейтральных сообщений:** `{sent_stat.get('neutral', 0):,}` (`{sent_stat.get('neu_ratio', 0):.1f}%`)")
        md_lines.append(f"- **Вопросительных сообщений (Q&A potential):** `{sent.get('questions_count', 0):,}` (`{sent.get('questions_ratio_percentage', 0):.1f}%`)")
        md_lines.append(f"- **Сообщений с фрагментами кода:** `{sent.get('code_snippets_count', 0):,}` (`{sent.get('code_snippets_ratio_percentage', 0):.1f}%`)")
        md_lines.append(f"- **Коротких сообщений (<20 симв):** `{noise.get('short_messages_ratio_percentage', 0):.1f}%`")
        md_lines.append(f"- **Пустых / без слов сообщений:** `{noise.get('empty_messages_ratio_percentage', 0):.1f}%`\n")
    ...
    def print_terminal_summary(self):
        ...
        print(f"   Период: {v.get('date_start', '')[:10]} — {v.get('date_end', '')[:10]} ({v.get('total_days_active', 0)} дней)")
        print(f"   Общее число слов: {v.get('total_words', 0):,}")
        print(f"   Оценка токенов: {v.get('total_tokens_estimated', 0):,} (~{v.get('total_tokens_estimated', 0)/1e6:.2f}M)")
