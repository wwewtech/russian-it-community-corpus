...
# 🚀 Russian IT Community Conversational Corpus (2018–2026)

## 📌 Описание датасета (Dataset Summary)

**Russian IT Community Conversational Corpus** — это масштабный, деидентифицированный (Zero-PII) и размеченный корпус технических диалогов из ведущих публичных русскоязычных сообществ инженеров, архитекторов, фаундеров и ML-разработчиков (*«Русский ИТ бизнес»*, *«XOR CHAT»* и др.) за **8 лет непрерывной истории (2018–2026 гг.)**.

Датасет очищен от спама, бот-уведомлений и шума, структурирован по ориентированным диалоговым графам (Thread DAG) и экспортирован в **5 стандартизированных форматов машинного обучения**:
- **Apache Parquet (`.parquet`)**: сжатый бинарный формат с компрессией `zstd`.
- **ShareGPT JSONL (`sft_sharegpt_format.jsonl`)**: многоходовые диалоги для `Axolotl`, `FastChat`, `LLaMA-Factory`.
- **Alpaca Instruction JSONL (`sft_alpaca_format.jsonl`)**: пары `{"instruction", "input", "output"}`.
- **OpenAI ChatML JSONL (`sft_openai_messages.jsonl`)**: диалоги `{"messages": [{"role": "system", ...}, {"role": "user", ...}, {"role": "assistant", ...}]}` для `Unsloth` и `TRL`.
- **RAG Knowledge Base (`rag_chunks_kb.jsonl` & `.parquet`)**: структурированные базы знаний для векторных БД (`Qdrant`, `Chroma`, `pgvector`).
- **DPO Preference Pairs (`dpo_preference_pairs.jsonl`)**: пары предпочтений `{"prompt", "chosen", "rejected"}` для прямого обучения предпочтениям.
...
| :--- | :--- | :--- |
| **Всего сообщений** | `500,000+` | Очищенные сообщения после дедупликации |
| **Уникальных участников** | `9,500+` | Анонимизированные авторы (`Developer_XXXXX`) |
| **Период сбора** | `17.11.2018 — 22.08.2026` | 8 лет непрерывной хронологии IT-индустрии |
| **Суммарный объём слов** | `~7.5M слов` | Лемматизированный русскоязычный технический корпус |
| **Оценка объёма токенов (BPE)** | `~10.2M токенов` | Tiktoken `cl100k_base` / LLaMA-3 BPE |
| **SFT диалоговых веток** | `25,000+` | Многоходовые диалоги с подтвержденным качеством |
| **DPO пар предпочтений** | `3,500+` | Сравнения качественных и слабых ответов |
| **RAG чанков базы знаний** | `18,000+` | Сегментированные документы с метаданными |
| **Индекс Шеннона (Diversity)** | `13.8+` | Высокое лексическое разнообразие |
| **Доля вопросов (Q&A potential)** | `~20.5%` | Живые проблемно-ориентированные дискуссии |
