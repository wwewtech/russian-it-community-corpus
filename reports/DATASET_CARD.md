...
**RICC** — масштабный деидентифицированный корпус технических сообщений из русскоязычных сообществ разработчиков, архитекторов, основателей стартапов и ML-инженеров за период с августа 2017 по август 2026 года (2.65M сырых записей из 5 сообществ).
...
- **Alpaca JSONL**: пары инструкций и ответов (876k+ пар)
- **OpenAI ChatML JSONL**: диалоги сообщений для Unsloth и TRL (150k+ диалогов)
- **RAG Knowledge Base**: база знаний для векторных систем поиска Qdrant, Chroma и pgvector (282k+ чанков)
- **DPO Preference Pairs**: пары предпочтений для оптимизации ответов модели (55k+ пар)
...
| :--- | :--- | :--- |
| **Очищенных сообщений** | `2 558 479` | Сообщения после дедупликации и деидентификации |
| **Участников** | `203 682` | Псевдонимизированные авторы |
| **Период сбора** | `06.08.2017 — 22.08.2026` | 9 лет непрерывной хронологии |
| **Суммарный объём слов** | `34 040 025` | Технический словарный корпус |
| **Оценка объёма токенов** | `~44.84M токенов` | Tiktoken cl100k и LLaMA-3 BPE |
| **SFT диалоговых веток** | `150 749` | Диалоги с подтвержденным качеством |
| **DPO пар предпочтений** | `55 895` | Пары для обучения предпочтениям |
| **RAG чанков базы знаний** | `282 777` | Сегментированные документы с метаданными |
